<template>
  ...
  <div
    class="collapse-wrap flex items-center justify-between"
    :class="{ 'collapse-show': state.visible }"
  >
    <template v-for="(item, index) in options" :key="index">
      <div
        class="collapse-item"
        :class="{ gutter: index !== 0, active: state.value === item.value }"
        @focus="handleFocus"
        @blur="handleBlur"
        :tabindex="index"
